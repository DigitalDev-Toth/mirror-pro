...
module.exports = function () {
    ...
    this.When(/^I press four times the "add desks" button and next one time the "remove desks" button$/, 
        function(next) {
    ...
    this.Then(/^I should see four "desks" into the layout$/, function(next) {
    ...
    this.Then(/^I can resize the edges of the "desks"$/, function(next) {
    	...
    	this.driver
    		.moveToObject( "#panel-layout", 700, 428 )
    		.buttonDown( 0 )
    		.moveToObject( "#panel-layout", 100, 100 )
    		.buttonUp( 0 )
    		.execute(() => {
    			return parseInt( document.getElementById( "panel-layout" ).children[0].children[0].style.width )
    		})
    		.then((result) => {
    			expect( result.value ).to.above(199);
    		})
    		.call( next );
    });
    ...
    this.Then(/^I should see the predefined layout selected on the "panel-layout"$/, function(next) {
